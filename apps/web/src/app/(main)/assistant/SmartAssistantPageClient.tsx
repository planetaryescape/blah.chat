  ...
  const handleAudioUpload = async () => {
    ...
    try {
      const { uploadUrl, storageId } = await apiClient.post<{
        ...
        fileName: audioFile.name || `meeting-audio-${Date.now()}.webm`,
      ...
      const { jobId } = await sdk.transcribeAudio({
        storageId,
        mimeType: audioFile.type,
      });
      const job = await sdk.waitForJob<{ text: string }>(jobId, {
        timeoutMs: 95_000,
        initialInterval: 1000,
        maxInterval: 10_000,
      });
      const transcriptResult = job.result?.text;
      if (!transcriptResult) {
