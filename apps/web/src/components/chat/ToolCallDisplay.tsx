...
export function ToolCallDisplay({
  ...
  const shouldCollapseSummary = uniqueCalls.length > 1;
  const showCalls = !shouldCollapseSummary || summaryExpanded;
    ...
    <div className="my-3">
      {shouldCollapseSummary && (
        <button
          ...
          className="mb-2 flex w-full items-center justify-between rounded-lg border border-border/60 bg-muted/30 px-3 py-2 text-left text-xs text-muted-foreground transition-colors hover:text-foreground"
        >
          <span>
      ...
      {showCalls && uniqueCalls.length > 1 && (
