...
import { FolderGit2, Plus, Save, Star, Trash2 } from "lucide-react";
...
import { type ProjectNote, useNotes } from "@/hooks/useNotes";
...
export function NotesWorkspace() {
  ...
  const selectedNote = useMemo(
    () => notes.find((note) => note._id === noteParam) ?? null,
    [noteParam, notes],
  );

  useEffect(() => {
    if (noteParam && !selectedNote && notes.length > 0) {
      void setNoteParam(null);
    }
  }, [noteParam, notes.length, selectedNote, setNoteParam]);
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-background">
      <div
        className={cn(
          ...
          noteParam && isMobile ? "hidden" : "flex",
        )}
      >
                ...
                {notes.map((note) => (
                  <button
                    ...
                    className={cn(
                      ...
                      noteParam === note._id && "bg-secondary",
                    )}
                  >
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This removes the note from the Postgres store.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
