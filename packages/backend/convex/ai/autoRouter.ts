...
function getSpeedBonus(modelId: string): number {
	...
	// Check hostOrder for fast inference providers
	if (config.hostOrder?.includes("cerebras")) return 30;
	if (config.hostOrder?.includes("groq")) return 25;

	// Check model name for speed indicators
	if (modelId.includes("flash") || modelId.includes("fast")) return 20;
	if (modelId.includes("nano") || modelId.includes("lite")) return 25;
	if (modelId.includes("lightning")) return 28;

	// Reasoning models are slower
	if (config.capabilities.includes("thinking")) return -10;
	if (config.capabilities.includes("extended-thinking")) return -15;
