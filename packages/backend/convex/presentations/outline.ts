...
export const parseOutlineMessageInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingItems) {
      ...
      // Mark all items as complete (they were "partial" during streaming)
      (await (ctx.runMutation as any)(
        // @ts-ignore - TypeScript recursion limit with 94+ Convex modules
        internal.outlineItems.markComplete,
        { presentationId: args.presentationId },
      )) as Promise<void>;

      // Update status to outline_complete
      await ctx.db.patch(args.presentationId, {
        status: "outline_complete",
        currentOutlineVersion: 1,
        outlineStatus: "ready",
    ...
    // Create outlineItems with "complete" status (final parsing)
    const now = Date.now();
    for (const slideData of parsedSlides) {
      await ctx.db.insert("outlineItems", {
        presentationId: args.presentationId,
