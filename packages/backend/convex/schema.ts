...
export default defineSchema({
  ...
  memories: defineTable({
    ...
    .index("by_retention", ["userId", "retention"]) // Query by retention (fading memories)
    .index("by_stability", ["userId", "stability"]) // Query by stability (promotion candidates)
    .vectorIndex("by_embedding", {
      vectorField: "embedding",
      dimensions: 1536,
      filterFields: ["userId"],
  ...
  // Cognitive Memory: Associative links between memories
  memoryLinks: defineTable({
    sourceId: v.id("memories"),
    targetId: v.id("memories"),
    strength: v.number(), // 0.0-1.0, grows when co-accessed
    createdAt: v.number(),
    updatedAt: v.number(),
  })
