...
import type { Memory, MemoryInput, MemoryLink, MemoryType, ScoredMemory } from '../core/types';
...
export abstract class MemoryAdapter {
   ...
   * 
   * @param memoryId Memory ID
   * @returns Array of links where this memory is source or target
   */
  abstract getLinks(memoryId: string): Promise<MemoryLink[]>;
...
/**
 * Base implementation with transaction no-op
 * Concrete adapters can extend this instead of implementing everything
 */
export abstract class BaseMemoryAdapter extends MemoryAdapter {
  /**
   * Default transaction implementation (no-op)
   * Override if your database supports transactions
   */
  async transaction<T>(
    callback: (adapter: MemoryAdapter) => Promise<T>
  ): Promise<T> {
    // Default: just execute the callback without transaction
