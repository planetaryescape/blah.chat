 ...
 * (see cognitive-memory-skill repo for gpt-5-mini implementation).
 * 
 * Heuristics:
...
export function scoreImportance(text: string): number {
  ...
  // Decision words (+0.15)
  const decisionWords = ['decided', 'chose', 'will', 'must', 'should', 'need to', 'plan to'];
  ...
  const temporalWords = ['yesterday', 'tomorrow', 'next week', 'next month', 'deadline'];
  if (temporalWords.some(word => lowerText.includes(word))) {
    score += 0.1;
  }
  
  // Strong sentiment words (+0.15)
  const sentimentWords = ['love', 'hate', 'amazing', 'terrible', 'critical', 'urgent'];
 ...
 * @returns Memory type
 */
export function categorizeMemoryType(text: string): 'episodic' | 'semantic' | 'procedural' {
  const lowerText = text.toLowerCase();
  
  // Procedural indicators
  const proceduralWords = ['how to', 'step 1', 'first', 'then', 'next', 'finally', 'instructions'];
...
export function extractTopics(text: string, maxTopics: number = 5): string[] {
  ...
  const stopWords = new Set([
    ...
    'could', 'can', 'may', 'might', 'must', 'i', 'you', 'he', 'she', 'it', 'we', 'they'
  ]);
