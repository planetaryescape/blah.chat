...
export default function NewSlidesPage() {
  ...
  const [slideStyle, setSlideStyle] = useState<"wordy" | "illustrative">("illustrative");
  ...
  const [selectedTemplateId, setSelectedTemplateId] = useState<Id<"designTemplates"> | null>(null);
  ...
  const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type === "application/pdf") {
      toast.info("PDF upload coming soon. Please copy-paste the text content for now.");
      return;
    }

    if (file.type.includes("wordprocessingml")) {
      toast.info("DOCX upload coming soon. Please copy-paste the text content for now.");
      return;
    }
  }, []);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const styleLabel = slideStyle === "illustrative" ? "illustrative (Speaker Assist)" : "wordy (Self-Contained)";
            ...
            <div className="flex justify-center">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="mode-prompt" className="cursor-pointer font-medium">Generate from Idea</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="outline" id="mode-outline" />
                  <Label htmlFor="mode-outline" className="cursor-pointer font-medium">I Have an Outline</Label>
                </div>
              </RadioGroup>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                   <Label htmlFor="input" className="text-base font-semibold">
                    {inputMode === "prompt" ? "Describe your presentation" : "Paste your outline"}
                  </Label>
          ...
          <div className="grid gap-8 md:grid-cols-2">
             ...
             <div className="space-y-3">
              <Label htmlFor="title" className="text-sm font-medium">
                  Presentation Title <span className="text-muted-foreground font-normal">(Optional)</span>
              </Label>
                 ...
                 <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={cn(
                                 "p-1.5 rounded-md",
                                 slideStyle === "illustrative" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                             )}>
                                <Mic className="h-4 w-4" />
                             </div>
                            <div>
                                <span className="text-sm font-medium block">Speaker Assist</span>
                                <span className="text-[10px] text-muted-foreground block">Visual slides, detailed notes</span>
                            </div>
                        </div>
                        {slideStyle === "illustrative" && (
                    ...
                    >
                         <div className="flex items-center gap-3">
                            <div className={cn(
                                 "p-1.5 rounded-md",
                                 slideStyle === "wordy" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                             )}>
                                <FileIcon className="h-4 w-4" />
                             </div>
                            <div>
                                <span className="text-sm font-medium block">Self-Contained</span>
                                <span className="text-[10px] text-muted-foreground block">Detailed slides, standalone</span>
                            </div>
                        </div>
                         {slideStyle === "wordy" && (
                            <CheckCircle2 className="h-4 w-4 text-primary" />
            ...
            <CollapsibleTrigger asChild>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={cn(
                    "p-2 rounded-lg",
                    selectedTemplateId ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                  )}>
                    <Palette className="h-4 w-4" />
                  </div>
                  <div>
                    <span className="text-sm font-medium block">Brand Template</span>
                    <span className="text-xs text-muted-foreground">
                      {selectedTemplateId ? "Template selected" : "Match your organization's branding"}
                    </span>
                  </div>
                </div>
