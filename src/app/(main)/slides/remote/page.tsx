...
function RemoteControlContent() {
	...
	const slides = useQuery(
		...
		session?.presentationId ? { presentationId: session.presentationId } : "skip",
	);
	...
	const formatTimer = (startedAt?: number, elapsed?: number) => {
		...
		const totalMs = startedAt ? Date.now() - startedAt + (elapsed || 0) : elapsed || 0;
		...
		<div className="h-[100dvh] flex flex-col bg-background overflow-hidden">
			...
			<div className="py-2 px-3 text-center border-t shrink-0">
				<div className="text-base font-medium">
					Slide {(session?.currentSlide ?? 0) + 1} of {session?.totalSlides ?? 0}
				</div>
				{currentSlide?.title && (
			...
			<div className="p-3 grid grid-cols-2 gap-3 shrink-0">
				...
				<Button
					...
					className="h-20 text-lg"
					onClick={handleNext}
					disabled={(session?.currentSlide ?? 0) >= (session?.totalSlides ?? 1) - 1}
				>
					Next
