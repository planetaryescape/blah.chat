...
export function NoteEditor({ noteId }: NoteEditorProps) {
  ...
  const performSave = useCallback(async (content: string, currentTitle: string) => {
    setIsSaving(true);
    try {
      await updateNote({
        noteId,
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b bg-background/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-2 text-muted-foreground text-sm">
           {lastSaved ? (
              <span className="flex items-center gap-1.5 opacity-70">
                 {isSaving ? <Loader2 className="h-3 w-3 animate-spin"/> : <Clock className="h-3 w-3" />}
        ...
        <div className="flex items-center gap-1">
             <Button
              ...
              >
                  <Pin className={cn("h-4 w-4", note.isPinned && "fill-current text-orange-500")} />
              </Button>
                    ...
                    <div className="flex items-center gap-2 hover:text-foreground transition-colors">
                        ...
                        <Select
                            ...
                            await updateNote({
                                ...
                                toast.success(value === "none" ? "Removed from project" : "Moved to project");
                            }}
                        >
