...
export function NoteListItem({ note, isSelected, onClick }: NoteListItemProps) {
  ...
  return (
    <div className="relative">
        ...
        <div className="flex items-baseline justify-between gap-2 mb-1">
          <h3
             className={cn(
                ...
                isSelected || note.isPinned ? "font-semibold text-foreground" : "font-medium text-foreground/90"
             )}
          >
      ...
      {!isSelected && <div className="absolute bottom-0 left-4 right-0 h-px bg-border/40" />}
    </div>
  );
}
